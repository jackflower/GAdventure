...
var bullet_data = preload("DefensiveTowerBulletDelta.tscn")
	...
	if(my_target):
		if (tower_process_enabled and target_reference.get_ref()):
			...
			if(shooting and not prev_shooting and can_shoot_animation):
				create_bullet()
				$AnimationPlayerTowerDelta.play("tower_delta_animation")
				$AnimationPlayerTowerDelta.playback_speed = shot_speed
